...
func defence(charName, charСlass string) string {
...
func startTraining(charName, charСlass string) string {
	if charСlass == "warrior" {
	...
	for cmd != "skip" {
		...
		if cmd == "attack" {
			fmt.Println(attack(charName, charСlass))
		}

		if cmd == "defence" {
			fmt.Println(defence(charName, charСlass))
		}

		if cmd == "special" {
			fmt.Println(special(charName, charСlass))
		}
	}
...
func main() {
	...
	charСlass := choiseCharСlass()

	fmt.Println(startTraining(charName, charСlass))
}
